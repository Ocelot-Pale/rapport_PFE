...
\subsubsection{La transformée multi-échelle}
    ...
    Cela requiert la notion de détail. Cette notion permet de ne pas utiliser la transformée en ondelette pour quantifier dans l'absolu le contenu porté par une échelle particulière,
    mais plutôt à comprendre en quoi il s'éloigne de ce que les échelles supérieures pourraient laisser supposer. En clair à partir d'une résolution donnée $j$,
    on définit un \textbf{prédicteur} polynomial, qui tente d'inférer l'allure de la fonction au niveau $j+1$. Le \textbf{détail} ne cherche pas naïvement à quantifier et localiser
    l'information contenue aux échelles du niveau $j+1$ mais plutôt, à quantifier l'écart à la prédiction polynomiale.

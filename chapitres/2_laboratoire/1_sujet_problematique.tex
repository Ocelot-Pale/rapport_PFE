...
Comme détaillé plus tard, ces équations sont difficiles à simuler avec précision. L'objet du stage est donc de contribuer à l'élaboration
...
\subsection{Les difficultés des équations d'advection-diffusion-réaction}
    ...
    \begin{enumerate}[label=\Alph*.]
        \item des opérateurs aux propriétés différentes, antagonistes
    ...
    \subsubsection{Des opérateurs aux propritétés antagonistes}
        ...
        \end{itemize}
        Ainsi, une approche monolithique (qui traiterait tous les opérateurs d'un bloc) échouera souvent à simuler le système de manière satisfaisante.
    ...
    \footnote{Par exemple : création d'espèces intermédiaires à temps de vie court, couplage entre la réaction et la diffusion de chaleur...}
    et,
...
\subsection{Les stratégies de simulation des équations d'advection-diffusion-réaction}
    ...
    \begin{enumerate}[label=\Alph*.]
        \item Le découplage d'opérateurs, permet de développer des schémas non-monolithiques où chaque opérateur est traité indépendamment des autres avec une méthode adaptée à ses propriétés. 
    ...
    \subsubsection{Les stratégies d'adaptation en espace}
        ...
        grâce à une reconstruction par interpolation polynomiale (décompression/prédiction).\par 
% 
    ...
    \label{par:problematique}
        ...
        En effet il existe plusieurs stratégies d'adaptation par multirésolution adaptative (plus de détails en \ref{par:contrib_2:cadre}) : 
        \begin{itemize}
            \item \hlblue{\textit{L'approche MRA standard}} est de faire évoluer la solution sur le maillage adapté (compressé) à partir des valeurs de la solution adaptée - approche dite \emph{sans reconstruction des flux}.
            \item \hlperu{\textit{Une approche MRA non-standard}} est de faire évoluer la solution sur le maillage adapté, mais à partir des valeurs de la solution reconstruite, décompressée, plus précises - approche dite \emph{avec reconstruction des flux}.
        \end{itemize}
    ...
    sur les trois opérateurs combinés : advection, diffusion, réaction.}. Il se décompose en trois contributions se focalisant sur différents points du problème:
    \begin{enumerate}
